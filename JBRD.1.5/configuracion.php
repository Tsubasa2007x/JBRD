...
// ── Cargar datos actuales (JOIN cuentas + usuarios) ──────────────────────
$stmt = $pdo->prepare("
    SELECT u.*, c.usuario, c.creditos, c.experiencia, c.id_rol
    FROM usuarios u
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($accion === 'cambiar_password') {
        ...
        // Verificar contra tabla cuentas
        $chk = $pdo->prepare("SELECT password FROM cuentas WHERE id_cuenta = ?");
        $chk->execute([$id_cuenta]);
        ...
        if (empty($errores)) {
            ...
            $pdo->prepare("UPDATE cuentas SET password = ? WHERE id_cuenta = ?")->execute([$hash, $id_cuenta]);
...
.danger-zone h3 { font-family: 'Syne', sans-serif; font-size: .95rem; color: var(--danger); margin-bottom: .4rem; }
...
html.tema-claro .info-item {
  background: var(--bg2);
}
 
html.tema-claro .field input,
html.tema-claro .field select {
  background: var(--bg3);
  color: var(--text);
}
</style>
...
<div class="page">
  ...
  <aside class="sidebar">
    ...
    <a href="paginas/perfil.php" class="nav-link">
    ...
    <a href="php/cerrar_sesion.php" class="nav-link" style="color:var(--danger); margin-top:auto;">
    ...
    <div class="sidebar-footer">
      <div class="avatar-row">
        <div class="avatar"><?= strtoupper(substr($usuario['nombre'] ?? 'U', 0, 1) . substr($usuario['apellido'] ?? '', 0, 1)) ?></div>
  ...
  <main class="main">
    ...
    <div class="tab-panel active" id="tab-personal">
      ...
      <div class="info-grid">
        <div class="info-item">
          <div class="lbl">ID de cuenta</div>
          <div class="val">#<?= h($id_cuenta) ?></div>
        </div>
        <div class="info-item">
    ...
    <div class="tab-panel" id="tab-seguridad">
      ...
      <!-- CORREGIDO: eliminado el toggle duplicado/suelto que estaba aquí -->
      <div class="card">
        <div class="card-title">
        ...
        <div style="margin-top:.5rem">
          <a href="php/cerrar_sesion.php?all=1" class="btn btn-ghost" style="font-size:.85rem">Cerrar todas las sesiones</a>
        </div>
      </div>
    </div>
      ...
      <div class="card">
        ...
        <div class="toggle-row">
          <div class="toggle-info">
            ...
            <div class="toggle-desc" id="tema-desc">Actualmente usando el tema oscuro.</div>
          </div>
          <label class="switch">
            <input type="checkbox" id="toggle-tema" checked>
            <span class="switch-track"></span>
          </label>
        </div>
...
(function () {
  ...
  const desc     = document.getElementById('tema-desc');
  const temaGuardado = localStorage.getItem('tema');
 
  // Estado inicial: checked = oscuro, unchecked = claro
  if (temaGuardado === 'claro') {
    toggle.checked = false;
    if (desc) desc.textContent = 'Actualmente usando el tema claro.';
  } else {
    toggle.checked = true;
    if (desc) desc.textContent = 'Actualmente usando el tema oscuro.';
  }
