...
document.addEventListener('DOMContentLoaded', function () {
  ...
  /* =========================================
     JBRD LIVE MARKET — TRADINGVIEW WIDGETS
     
     CORRECCIONES APLICADAS:
     1. Se usa el método de "Embed Widget" vía script inyectado,
        que es la forma más robusta y actualizada de TradingView.
     2. Ya no se depende de new TradingView.widget() (API antigua),
        sino del snippet oficial que TradingView recomienda.
     3. Los contenedores tienen la estructura correcta:
...
function loadBTCWidget() {
  ...
  // La configuración se pasa como texto interno del script
  script.innerHTML = JSON.stringify({
    "autosize":          true,
    "symbol":            "BINANCE:BTCUSDT",
    "interval":          "5S",           // velas de 15 min en vivo
...
function loadDXYWidget() {
  ...
  script.innerHTML = JSON.stringify({
    "autosize":          true,
    "symbol":            "CAPITALCOM:DXY",      // Dollar Index oficial de TradingView
    "interval":          "5S",
    "timezone":          "America/Bogota",
    "theme":             "dark",
    "style":             "1",
...
// Definición de mercados: nombre, zona horaria, hora apertura y cierre
var MERCADOS = [
  ...
  { nombre: 'Crypto',   zona: null,                  abre:  0,    cierra: 24   }, // 24/7
];
...
function getHoraDecimal(zona) {
  var now = new Date();
  if (!zona) return now.getHours() + now.getMinutes() / 60; // local para crypto
...
(function () {
  ...
  // Pequeño delay para que la transición CSS sea visible al cargar
