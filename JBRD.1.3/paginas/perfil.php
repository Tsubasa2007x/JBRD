...
// correo y datos personales viven en `usuarios`, créditos/XP en `cuentas`
$stmt = $conn->prepare("
    SELECT c.usuario,
           c.creditos,
           c.experiencia,
...
// Lecciones completadas  →  progreso_lecciones (completado = 1)
$stmtL = $conn->prepare("
...
// Misiones completadas  →  cuenta_misiones (completada = 1)
$stmtM = $conn->prepare("
    SELECT COUNT(*) AS total
...
// Valores finales (BD tiene prioridad, sesión como fallback)
...
<head>
  ...
  <style>
    ...
    .xp-bar-fill {
      ...
      width: 0;                   /* empieza en 0; JS anima hasta data-pct */
