...
<head>
    ...
    <style>
        ...
        /* Placeholder mientras carga / no hay video_id */
        .video-placeholder {
            position: absolute;
            top: 0; left: 0;
            width: 100%; height: 100%;
        ...
        .lista-conceptos li {
            background: #16233e; border: 1px solid #223;
...
<body>
    ...
    <script>
            ...
            fetch(`get_contenidos.php?id_leccion=${idLeccion}`)
                ...
                .then(data => {
                    ...
                    contenidosActuales.forEach((cont, index) => {
                        ...
                        panel.innerHTML = construirPanel(cont, index);
        ...
        // ── Construye el HTML rico de cada panel ─────────────────────────────
        function construirPanel(cont, index) {
            // Video: el campo `video_id` en la BD debe contener el ID de YouTube
            // (ej. "dQw4w9WgXcQ"). Si no existe o está vacío, muestra placeholder.
            ...
            // Conceptos clave: el campo `conceptos` puede ser JSON array de strings
            // o texto separado por "|". Si no existe, no se muestra la sección.
            let conceptosHtml = '';
            if (cont.conceptos) {
                let items = [];
                try {
                    items = JSON.parse(cont.conceptos); // preferido: ["Concepto A","Concepto B"]
                } catch (_) {
                    items = cont.conceptos.split('|').map(s => s.trim()).filter(Boolean);
                }
                if (items.length > 0) {
                    const liItems = items.map(c =>
                        `<li><span class="bullet">▸</span>${escHtml(c)}</li>`
                    ).join('');
                    conceptosHtml = `
        ...
        function cambiarContenido(nuevoIndex) {
            ...
            // Scroll suave al inicio del visor al cambiar de parte
        ...
        function enviarProgreso() {
            ...
            fetch('../php/lecciones_procesar.php', { method: 'POST', body: formData })
                .then(r => r.json())
                .then(data => {
                    if (data.status === 'success') {
                        Swal.fire({
                            title: '¡Módulo Completado!',
                            html: `Felicidades, has adquirido nuevos conocimientos financieros.<br><br>
                            ...
                            document.getElementById('ui-rol').textContent = data.nuevo_rol;
